...
export function AppManager({ apps }: AppManagerProps) {
  // Simple identity function - no i18n needed for RayOS
  ...
  useEffect(() => {
    const handleAppLaunch = (
      ...
      if (initialPath) {
        localStorage.setItem(`rayos:app:${appId}:initial-path`, initialPath);
      }
  ...
  // No-op placeholders for removed dashboard functionality
  ...
  return (
    <>
      ...
      {Object.values(instances).map((instance) => {
        ...
        // Skip stickies in expose mode (no longer applicable)
