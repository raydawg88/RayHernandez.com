...
export function useControlPanelsLogic({
  ...
  const performReset = () => {
    ...
    const fileMetadataStore = localStorage.getItem("rayos:files");
    ...
    if (fileMetadataStore) {
      localStorage.setItem("rayos:files", fileMetadataStore);
    }
    if (usernameRecovery) {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      const timestamp = new Date()
        ...
        .join("-")
        .slice(0, -5);
      a.download = `rayos-backup-${timestamp}.gz`;
  ...
  const performRestore = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (backup.localStorage["rayos:app:settings:wallpaper"]) {
          const wallpaper = backup.localStorage["rayos:app:settings:wallpaper"];
          if (wallpaper) {
        ...
        try {
          ...
          const persistedKey = "rayos:files";
