...
import { ViewType, SortType } from "./FinderMenuBar";
...
const ListRowItem = memo(function ListRowItem({
  ...
  return (
    <TableRow
      className={`border-none transition-colors cursor-default`}
      style={
    ...
    >
      <TableCell className="flex items-center gap-2 py-0.5 border-r border-black/20">
        {file.icon &&
      ...
      <TableCell className="whitespace-nowrap py-0.5 border-r border-black/20">
        {file.size
          ? file.size < 1024
            ? `${file.size} B`
            : file.size < 1024 * 1024
      ...
      <TableCell className="py-0.5 border-r border-black/20">{getFileType(file)}</TableCell>
    ...
    return (
      ...
      >
        <Table className="min-w-[480px]">
          <TableHeader>
            <TableRow className="text-[11px] border-none font-normal" style={{ borderBottom: "1px solid #000" }}>
              <TableHead
                className={`font-normal h-[20px] px-2 py-0 border-r border-black cursor-default select-none ${sortType === "name" ? "underline" : ""}`}
                onClick={() => onSortTypeChange?.("name")}
              >
                {t("apps.finder.tableHeaders.name")}
              </TableHead>
              <TableHead
                className={`font-normal h-[20px] px-2 py-0 border-r border-black whitespace-nowrap cursor-default select-none ${sortType === "size" ? "underline" : ""}`}
                onClick={() => onSortTypeChange?.("size")}
              >
                {t("apps.finder.tableHeaders.size")}
              </TableHead>
              <TableHead
                className={`font-normal h-[20px] px-2 py-0 border-r border-black cursor-default select-none ${sortType === "kind" ? "underline" : ""}`}
                onClick={() => onSortTypeChange?.("kind")}
              >
                {t("apps.finder.tableHeaders.kind")}
              </TableHead>
              <TableHead
                className={`font-normal h-[20px] px-2 py-0 whitespace-nowrap cursor-default select-none ${sortType === "date" ? "underline" : ""}`}
