...
const UUID_MIGRATION_KEY = "rayos:indexeddb-uuid-migration-v1";
...
export function useFileSystem(
  ...
  const formatFileSystem = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("rayos:file-size-timestamp-sync-v1");
  ...
  useEffect(() => {
    const syncFileSizesAndTimestamps = async () => {
      ...
      const syncKey = "rayos:file-size-timestamp-sync-v1";
