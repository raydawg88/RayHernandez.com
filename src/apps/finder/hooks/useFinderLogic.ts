...
export function useFinderLogic({
  ...
  useEffect(() => {
    ...
    const storedPath =
      localStorage.getItem("rayos:app:finder:initial-path") ||
    ...
    if (storedPath) {
      localStorage.removeItem("rayos:app:finder:initial-path");
  ...
  const fileMenuItems = (file: FileItem): MenuItem[] => {
    const isInTrash = file.path.startsWith("/Trash") || currentPath === "/Trash";

    if (isInTrash) {
      return [
        {
          type: "item",
          label: t("apps.finder.menu.putBack", "Put Back"),
