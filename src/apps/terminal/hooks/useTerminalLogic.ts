...
export const useTerminalLogic = ({
  ...
  const appendAiMessage = (_msg: unknown, _opts?: unknown) => {};
  const isAiLoading = false;
  const stopAiResponse = () => {};
  const setAiChatMessages = (_msgs: unknown[]) => {};
  ...
  const processCommand = async (
    ...
    switch (cmd) {
      ...
      case "su": {
        ...
        return { output: "su: command not available", isError: true };
      }

      case "logout": {
        return { output: "logout: command not available", isError: true };
      }
