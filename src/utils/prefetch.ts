...
const MANIFEST_KEY = 'rayos:manifest-timestamp';
const LEGACY_MANIFEST_KEY = 'rayos-manifest-timestamp';
...
function getStoredVersion(): { version: string | null; buildNumber: string | null } {
  ...
  return {
    version: state.rayOSVersion,
    buildNumber: state.rayOSBuildNumber,
  };
}
...
function storeVersion(version: string, buildNumber: string, buildTime?: string): void {
  useAppStore.getState().setRayOSVersion(version, buildNumber, buildTime);
  ...
  if (result.action === 'none') {
    if (isManual) {
      ...
      toast.success('Already running the latest version', {
        description: stored.version ? `RayOS ${stored.version} (${stored.buildNumber})` : undefined,
      });
    }
    return;
  }
...
export function initPrefetch(): void {
  ...
  if (url.searchParams.has('_cb')) {
    ...
    try {
      sessionStorage.removeItem('rayos-stale-reload');
    } catch {
