...
const MIGRATION_KEY = "rayos:indexeddb-uuid-migration-v1";
const BACKUP_KEY = "rayos:indexeddb-backup";
...
async function backupDataBeforeMigration() {
  ...
  try {
    ...
    const db = await new Promise<IDBDatabase>((resolve, reject) => {
      const request = indexedDB.open("RayOS", 4); // Open version 4 explicitly
...
export async function migrateIndexedDBToUUIDs() {
  ...
  const currentDBVersion = await new Promise<number>((resolve) => {
    const request = indexedDB.open("RayOS");
    request.onsuccess = () => {
