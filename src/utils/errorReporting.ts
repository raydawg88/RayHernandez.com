...
export const RAYOS_ERROR_BOUNDARY_TEST_EVENT = "rayos:error-boundary-test";
...
declare global {
  interface Window {
    __RAYOS_ERROR_REPORTER__?: RuntimeErrorReporter;
...
export function getRuntimeErrorReporter(): RuntimeErrorReporter | null {
  ...
  if (typeof window !== "undefined") {
    return window.__RAYOS_ERROR_REPORTER__ ?? null;
  }
...
export function reportRuntimeCrash(
  ...
  console.error("[RayOS] Runtime crash caught by error boundary", {
    ...analyticsPayload,
  ...
  try {
    ...
    console.error("[RayOS] Failed to track runtime crash analytics", analyticsError);
  }
  ...
  if (reporter) {
    void Promise.resolve(reporter.reportError(report)).catch(
      (reporterError: unknown) => {
        console.error("[RayOS] Optional error reporter failed", reporterError);
      },
    );
  }
  ...
  if (typeof window !== "undefined" && typeof window.reportError === "function") {
    try {
      ...
      console.error("[RayOS] window.reportError failed", browserError);
    }
  }
}
...
export function triggerRuntimeCrashTest(detail: RuntimeCrashTestDetail): void {
  ...
  window.dispatchEvent(
    new CustomEvent<RuntimeCrashTestDetail>(RAYOS_ERROR_BOUNDARY_TEST_EVENT, {
      detail,
    }),
  );
}
