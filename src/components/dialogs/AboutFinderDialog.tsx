...
export function AboutFinderDialog({
  ...
  const version = useAppStore((state) => state.rayOSVersion);
  const buildNumber = useAppStore((state) => state.rayOSBuildNumber);
  const buildTime = useAppStore((state) => state.rayOSBuildTime);
          ...
          <div className="flex flex-row items-center space-x-2 p-2 px-4">
            ...
            >
              <div>
                ...
                >
                  <p>© Ray Hernandez. 1992-{new Date().getFullYear()}</p>
                </div>
              </div>
            </div>
          </div>
