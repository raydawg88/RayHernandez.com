...
import { AppId, getAppIconPath, appRegistryLookup, getNonFinderApps } from "@/config/appRegistry";
...
function MacDock() {
  ...
  const handleDividerDrop = useCallback((e: React.DragEvent) => {
    ...
    try {
      ...
      if (appId && appRegistryLookup[appId as AppId]) {
    ...
    (appId: AppId, specificInstanceId?: string): MenuItem[] => {
      ...
      if (appInstances.length > 0) {
        appInstances.forEach((inst) => {
          let windowLabel = inst.displayTitle || inst.title || appRegistryLookup[appId]?.name || appId;
