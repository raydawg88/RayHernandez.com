...
import { SpeakerSimpleLow, SpeakerSimpleHigh, SpeakerSimpleSlash, Gear, CaretUp, MagnifyingGlass } from "@phosphor-icons/react";
...
const finderMetadata = {
  ...
  creator: {
    name: "Ray Hernandez",
    url: "https://rayhernandez.com",
  },
  github: "https://github.com/raydawg88/RayHernandez.com",
  icon: "/icons/mac.png",
};
        ...
        <MenubarContent align="start" sideOffset={1} className="px-0">
          <MenubarItem
            ...
            onSelect={(e) => {
              ...
              console.log("[MenuBar] Undo stack before pop:", JSON.stringify(stack));
              const action = useUndoStore.getState().pop();
              console.log("[MenuBar] Popped action:", JSON.stringify(action));
              if (action?.type === "trash") {
                console.log("[MenuBar] Restoring item at path:", action.path);
                useFilesStore.getState().restoreItem(action.path);
              } else {
...
function VolumeControl() {
  ...
  const getVolumeIcon = () => {
    if (masterVolume === 0) {
      return <SpeakerSimpleSlash size={20} weight="fill" />;
    }
    if (masterVolume < 0.5) {
      return <SpeakerSimpleLow size={20} weight="fill" />;
    }
    return <SpeakerSimpleHigh size={20} weight="fill" />;
  };
    ...
    <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className={`h-8 w-8 text-md px-1 py-1 border-none focus-visible:ring-0 ${
            isXpTheme
