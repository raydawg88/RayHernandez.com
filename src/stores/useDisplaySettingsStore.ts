...
export const useDisplaySettingsStore = create<DisplaySettingsState>()(
  persist(
    (set, get) => ({
      ...
      // Wallpaper – default to empty (System 7 uses CSS background-color)
      currentWallpaper: "",
      wallpaperSource: "",
    ...
    {
      name: "rayos:display-settings",
      version: STORE_VERSION,
      migrate: (persisted: unknown, version: number) => {
        const state = persisted as Record<string, unknown>;
        if (version < 2) {
          // Reset photo wallpapers to empty (System 7 uses CSS background-color)
