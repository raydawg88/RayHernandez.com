...
export interface FileSystemItem {
  ...
  /** Desktop icon position (absolute, persisted) */
  desktopX?: number;
  desktopY?: number;
    ...
    (set, get) => ({
      ...
      ensureDefaultDesktopShortcuts: async () => {
        try {
          ...
          // RayOS desktop layout: folders, apps, and file shortcuts
          const desktopShortcuts: Array<{
            name: string;
            aliasTarget: string;
            aliasType: "app" | "file";
            appId?: string;
            ...
            // Folder shortcuts (open in Finder)
            { name: "About Ray", aliasTarget: "/Documents/About Ray", aliasType: "file", icon: "/icons/default/documents.png" },
            { name: "Projects", aliasTarget: "/Documents/Projects", aliasType: "file", icon: "/icons/default/documents.png" },
          ...
          // Batch create missing shortcuts
          const toCreate = desktopShortcuts.filter(
            (shortcut) =>
              !desktopItems.some(
                (item) =>
                  item.name === shortcut.name ||
                  (item.aliasTarget === shortcut.aliasTarget && item.aliasType === shortcut.aliasType)
              )
          );
          ...
          if (toCreate.length > 0) {
            set((currentState) => {
              ...
              for (const shortcut of toCreate) {
                const aliasPath = `/Desktop/${shortcut.name}`;
                if (newItems[aliasPath] && newItems[aliasPath].status === "active") {
                  continue;
                }
