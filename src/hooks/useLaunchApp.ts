...
// How long to show the watch cursor before opening the window
...
export const useLaunchApp = () => {
  ...
  const launchApp = (appId: AppId, options?: LaunchAppOptions) => {
    ...
    // Show the classic Mac OS watch cursor
    document.body.classList.add("os-busy");

    // Delay the actual launch so the user sees the watch cursor first
    setTimeout(() => {
      document.body.classList.remove("os-busy");
      doLaunch(appId, options);
    }, LAUNCH_DELAY_MS);
  };

  const doLaunch = (appId: AppId, options?: LaunchAppOptions) => {
