...
import { appRegistryLookup, getAppIconPath, getNonFinderApps } from "@/config/appRegistry";
...
const SEARCHABLE_COMMANDS = [
  ...
  { name: "ray", description: "Ask Ray AI", keywords: ["ai", "chat", "assistant"] },
];
...
export function useSpotlightSearch(query: string): SpotlightSearchState {
  ...
  const results = useMemo(() => {
    ...
    const appResults = allApps
      .filter((app) =>
        matchesQuery(
          ...
          app.name,
          appRegistryLookup[app.id]?.description
        )
      )
    ...
    results.push({
      id: "ai-ask-ray",
      type: "ai" as const,
      title: `${t("spotlight.askRay")} \u201C${trimmed}\u201D`,
      icon: getAppIconPath("terminal"),
      action: () => launchApp("terminal", { initialData: { prefillCommand: `ai ${trimmed}` } }),
    });
